...
func TestPostgreSQLUserRepository_CreateUser(t *testing.T) {
	...
	t.Run("successful creation", func(t *testing.T) {
		...
		rows := sqlmock.NewRows([]string{"id", "email"}).AddRow(userId, userEmail)
		mock.ExpectBegin()
		mock.ExpectQuery(`INSERT INTO "users"`).WithArgs(userEmail, sqlmock.AnyArg()).WillReturnRows(rows)
		mock.ExpectCommit()
	...
	t.Run("database error", func(t *testing.T) {
		...
		user := &entity.User{Email: userEmail}
		mock.ExpectBegin()
		mock.ExpectQuery(`INSERT INTO "users"`).WithArgs(userEmail, sqlmock.AnyArg()).WillReturnError(assert.AnError)
		mock.ExpectRollback()
	...
	t.Run("duplicate subscription constraint violation", func(t *testing.T) {
		...
		user := &entity.User{Email: userEmail}
		mock.ExpectBegin()
		mock.ExpectQuery(`INSERT INTO "users"`).WithArgs(userEmail, sqlmock.AnyArg()).WillReturnError(&pq.Error{Code: "23505", Message: "duplicate key value violates unique constraint"})
		mock.ExpectRollback()
...
func TestPostgreSQLUserRepository_GetAllUser(t *testing.T) {
	...
	t.Run("successful retrieval with users", func(t *testing.T) {
		...
		rows := sqlmock.NewRows([]string{"id", "email", "created_at"}).
			AddRow(1, "user1@example.com", createdAt1).
			AddRow(2, "user2@example.com", createdAt2)
...
func TestPostgreSQLUserRepository_GetUsersFromIds(t *testing.T) {
	...
	t.Run("successful getting users", func(t *testing.T) {
		...
		rows := sqlmock.NewRows([]string{"id", "email", "created_at"}).
			AddRow(1, "user1@example.com", createdAt1).
			AddRow(2, "user2@example.com", createdAt2).
			AddRow(3, "user3@example.com", createdAt3)
		mock.ExpectQuery(`SELECT \* FROM "users"`).
...
func TestPostgreSQLUserRepository_UpdateUser(t *testing.T) {
	...
	t.Run("successful updating user", func(t *testing.T) {
		...
		rows := sqlmock.NewRows([]string{"id", "email", "created_at"}).
			AddRow(userId, userEmail, createdAt)
		mock.ExpectBegin()
